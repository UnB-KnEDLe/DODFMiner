"""Regras regex para ato de Aviso de Ajdudicação de Licitação."""
...
class AdjudicacaoLicitacao(Atos):

    def __init__(self, file, backend):
        super().__init__(file, backend)

    def _act_name(self):
        return "Ajdudicação de Licitação"
    ...
    def _rule_for_inst(self):
        start = r"(AVISO(?:S)?\s+D[EO]\s+ADJUDICACAO\s+E\s+HOMOLOGACAO|AVISO(?:S)?\s+D[EO]\s+HOMOLOGACAO\s+E\s+ADJUDICACAO)"
