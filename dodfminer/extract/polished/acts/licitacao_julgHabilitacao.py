"""Regras regex para ato de Julgamento de Habilitação de Licitação."""
...
class JulgHabilitacaoLicitacao(Atos):

    def __init__(self, file, backend):
        super().__init__(file, backend)

    def _act_name(self):
        return "Julgamento de Habilitação de Licitação"
    ...
    def _rule_for_inst(self):
        start = r"(AVISO(?:S)?\s+D[EO]\s+JULGAMENTO\s+D[EO]\s+HABILITACAO)"
