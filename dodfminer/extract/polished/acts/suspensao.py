...
class Suspensao():
    ...
    def load(self):
        self.model = joblib.load('./models/suspensao.pkl')
        if self.file[-5:] == '.json':
            ...
            for orgao in section_3:
                for documento in section_3[orgao]:
                    for ato in section_3[orgao][documento]:
                        if "suspensão" in section_3[orgao][documento][ato]['titulo'].lower():
                            atos_suspensao.append(re.sub(r'<[^>]*>', '', section_3[orgao][documento][ato]['texto']))

        except KeyError:
            print("Chave 'Seção III' não encontrada")
            print(f"Chaves existentes: {file['json']['INFO'].keys()}")

        return atos_suspensao
