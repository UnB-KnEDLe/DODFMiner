"""Regras regex para ato de Recurso de Julgamento de Licitação."""
...
class RecursoJulgLicitacao(Atos):

    def __init__(self, file, backend):
        super().__init__(file, backend)

    def _act_name(self):
        return "Recurso de Julgamento de Licitação"
    ...
    def _rule_for_inst(self):
        start = r"(AVISO(?:S)?\s+D[EO]\s+RESULTADO\s+D[EO]\s+RECURSO\s+E\s+JULGAMENTO)"
