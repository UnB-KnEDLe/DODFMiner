"""Regras regex para ato de Nova Data Abertura de Licitação."""
...
class NovaDataLicitacao(Atos):

    def __init__(self, file, backend):
        super().__init__(file, backend)

    def _act_name(self):
        return "Nova Data de Abertura de Licitação"
    ...
    def _rule_for_inst(self):
        start = r"(AVISO(?:S)?\s+D[EO]\s+NOVA\s+DATA\s+D[EO]\s+ABERTURA|AVISO(?:S)?\s+NOVA\s+DATA\s+D[EO]\s+ABERTURA)"
